...
- Matrix metadata now updates `last_ok`/`status` after pipeline runs, but provider retry/backoff is still missing.
...
- Export a matrix status view (derived from persisted `last_ok`/`status`) to help spot stale series.
